...
func (conn *ServerConn) HandleSessionChannel(wg *sync.WaitGroup, newChan ssh.NewChannel) {
	...
	for req := range reqs {
		...
		case "shell":
			...
			dbg.Debug("Execute default shell")
			...
			return
		case "subsystem":
			dbg.Debug("Subsystem: %s", req.Payload[4:])
			...
			dbg.Debug("Trying to start sftp server")
			...
			if err != nil {
				dbg.Debug("sftp server can create",err)
			}
			if err := server.Serve(); err == io.EOF {
				server.Close()
				dbg.Debug("sftp client exited session.")
			} else if err != nil {
				dbg.Debug("sftp server completed with error:", err)
			}
			return
		case "exec":
			dbg.Debug("Execute exec command")
			execReq := &ExecRequest{}
